...
const Update = () => {
    ...
    const matchedEvent= events.find(e=>e._id === id)
    ...
    const handleUpdateEvents=async(event)=>{
        try {
        const {data}=await axios.patch(`/events/${id}`,event,{withCredentials:true})
        const updated= events.map(e=>e._id ===id? data:e)
        setevents(updated)
        ...
        navigate(`/`)
            ...
            console.log(error);
            toast.success("Error in Updating")
        }
    } 

  return (
    <div className='w-screen lg:h-screen h-[92vh] flex lg:flex-row flex-col-reverse lg:justify-center justify-evenly items-center p-4 lg:gap-2 gap-2 bg-gray-200'>
      <form className='lg:w-1/2 w-full flex flex-col items-center lg:bordr border-black rounded-2xl text-center lg:p-5 p-4 lg:h-fit h-fit lg:justify-center gap-4 bg-red-300 font-[montserrat] justify-center' action="" onSubmit={handleSubmit(handleUpdateEvents)}>
